...
def _build_session(
    ...
    # Keep retries low so a slow/unreachable endpoint fails fast instead of
    # stalling a research step for ~100s (5 retries x 20s timeout).
    retry = Retry(
        total=1,
        connect=1,
        read=1,
...
# ----- Per-process search cache (dedupes repeated searches within/across runs) -----
_SEARCH_CACHE: dict = {}


def _cache_key(tool: str, query: str, n: int) -> str:
    return f"{tool}|{n}|{(query or '').strip().lower()}"
...
def arxiv_search_tool(
    ...
    """
    Search arXiv and return lightweight results (title, authors, year, URL and
    abstract). PDF download + full-text extraction is intentionally OFF by
    default because it is slow (each PDF can take tens of seconds), which made
    the research step appear to hang. The abstract is enough for the writer to
    cite the work; flip the flags below if you need full text.
    """
    # ===== INTERNAL FLAGS =====
    _INCLUDE_PDF = False
    _EXTRACT_TEXT = False
    ...
    out: List[Dict] = []
    try:
        # (connect, read) timeouts: bail out quickly if arXiv is unresponsive.
        resp = session.get(api_url, timeout=(5, 8))
    ...
    try:
        ...
        for entry in root.findall("atom:entry", ns):
            ...
            out.append(item)
        if out and not any(isinstance(it, dict) and it.get("error") for it in out):
...
def tavily_search_tool(
    query: str, max_results: int = 4, include_images: bool = False
) -> list[dict]:
    """
    ...
    cache_key = _cache_key("tavily", query, max_results)
    if not include_images and cache_key in _SEARCH_CACHE:
        return _SEARCH_CACHE[cache_key]
...
def wikipedia_search_tool(query: str, sentences: int = 5) -> List[Dict]:
    ...
    try:
        ...
        result = [{"title": page.title, "summary": summary, "url": page.url}]
        _SEARCH_CACHE[_cache_key("wikipedia", query, sentences)] = result
        return result
    except Exception as e:
        return [{"error": str(e)}]
